# Build DC-05 (Channel Intensive Cultivation Driver Board) as slide 8 of the executable component deck.
...
$content = Get-Content -Raw -Encoding UTF8 (Join-Path $scriptRoot "dc05-content.json") | ConvertFrom-Json
$PT = 72.0
...
try {
    ...
    $slide = $presentation.Slides.AddSlide(8, $layout)
    ...
    for ($i = 0; $i -lt 3; $i++) {
        ...
        Add-Rect $slide ("LEVER_BAND_" + ($i + 1)) $x 1.75 $leverW 2.35 $fillInt $GREY
    ...
    Add-Txt $slide "CORE_TITLE" 0.85 4.58 2.6 0.5 $content.core_title 15 $true (16777215) 1
    Add-Txt $slide "CORE_FINDING" 3.6 4.62 8.9 0.9 $content.core_finding 15 $false (16777215) 1

    # Focus outlet pool
    Add-Txt $slide "POOL_TITLE" 0.6 5.9 2.4 0.5 $content.pool_title 15 $true $RED 1
    Add-Txt $slide "POOL_TEXT" 3.1 5.9 9.6 0.5 $content.pool_text 14 $false $INK 1

    Add-Txt $slide "TEACHING_CUE" 0.6 6.7 12.1 0.5 $content.cue 12 $false $GREY 1
    ...
    Write-Output "DC05_SLIDE_8_OK slides=$slideCount"
}
catch {
