...
- 【DNA 继承 gate】每一页的"视觉源"字段必须写清楚：匹配源页=复制；无源页=**继承哪个组件的什么 DNA（骨架/箭头/语义色）+ 模板母版**。说不清 DNA 的页不得 Build（防"长虹计划"式无根页面）。
- 模板母版继承是硬性停止条件：右上角品牌标识、页脚、背景必须继承 yuhong-template；出现任何第三方/自造标识（如"长虹计划"）→ 停止报告，不进入内容核验。
...
| 版本 | 日期 | 说明 |
|---|---|---|
| V0.6 | 2026-08-18 | 修正组件判断（P1 验出"长虹计划"无根页面）：组件体系 6 层共约 190 项，无源页≠无组件；逐页补 DNA 继承映射（P1→I1+DC-01 骨架、P3→DC-01 FROM→TO/语义色、P2/P4→I1+樱桃库参考）；新增 DNA 继承 gate 与"模板母版继承=硬性停止条件"。 |
