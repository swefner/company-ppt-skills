...
> **组件选择分工（V0.8，两 Agent 都要遵守）**：决策 A、证据 B、拍板 A。
> 1. A 出候选：每页 教学问题 → 组件候选 + DNA 映射（文本契约：catalog/slots/卡片，A 负责语义与铁律）；
> 2. **B 视觉核验（强制环节，不是可选）**：B 读图册/源页后，必须报告"布局事实 + 是否可承载当前内容 + 槽位是否可按名替换 + 插入后是否可编辑/主题是否完整带入"——A 无图臆断的错（漏层、认错视觉元素）靠 B 的视觉证据兜住；
> 3. A 定稿：结合 B 的视觉证据 + 语义铁律最终拍板；
> 4. B 执行：按定稿建页，不得自行改选组件。
> 背景：2026-08-18 实测——A 单独挑漏了互动组件层（I1-I8）并误判模板原生"长虹计划"为违规（无图臆断）；B 单独挑缺仓库语义铁律（曾有看对布局改写语义的记录）。任何一方单独挑都会翻车。

```
【交接包】Agent A（仓库侧）→ Agent B（PowerPoint 视觉执行）· V0.8
...
- 【授权例外】本任务四页均为 Agent A 判定的"无匹配源页定制页"，已书面授权：
...
| 版本 | 日期 | 说明 |
|---|---|---|
| V0.8 | 2026-08-18 | 明确组件选择分工：**决策 A、证据 B、拍板 A**；B 视觉核验（布局事实/承载能力/槽位可替换/可编辑性/主题带入）设为**强制环节**，防止 A 无图臆断；B 不得自行改选组件。 |
